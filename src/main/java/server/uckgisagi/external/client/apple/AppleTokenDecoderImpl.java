...
public class AppleTokenDecoderImpl implements AppleTokenDecoder {
    ...
    public String getSocialIdFromIdToken(@NotNull String idToken) {
        ...
        try {
            ...
            throw new ValidationException(String.format("만료된 애플 idToken (%s) 입니다 (reason: %s)", idToken, e.getMessage(), ErrorResponseResult.VALIDATION_AUTH_TOKEN_EXCEPTION));
            ...
            throw new ValidationException(String.format("잘못된 애플 idToken (%s) 입니다 (reason: %s)", idToken, e.getMessage(), ErrorResponseResult.VALIDATION_AUTH_TOKEN_EXCEPTION));
        }
    }
